...
def run(a):
    ...
    for fi,b in enumerate(folds,1):
        ...
        for mn in models:
            ...
            methods['opbgb_two_sided']=(opt.predict(pt),opt.params())
            # selection based only on calibration-select log loss
            candidates={
                'beta':be.predict(pcs),'bgb':bg.predict_proba(pcs)[:,1],
                'opbgb_symmetric':ops.predict(pcs),'opbgb_two_sided':opt.predict(pcs)}
            ...
            for cal,(pred,par) in methods.items():
                row={'fold':fi,'model':mn,'calibration':cal,'selected_method':selected if cal=='nested_selected' else '', 'n_test':len(yt),'prevalence':float(yt.mean()),'fit_time_model':fit_time,**assess(yt,clip01(pred),amt)}
                metrics.append(row); params.append({'fold':fi,'model':mn,'calibration':cal,**par})
    ...
    summary=md.groupby(['model','calibration'],as_index=False).agg(pr_auc_mean=('pr_auc','mean'),pr_auc_sd=('pr_auc','std'),brier_mean=('brier','mean'),logloss_mean=('logloss','mean'),ece_mean=('ece','mean'),ici_mean=('ici','mean'),cal_intercept_mean=('cal_intercept','mean'),cal_slope_mean=('cal_slope','mean'),oe_ratio_mean=('oe_ratio','mean'))
